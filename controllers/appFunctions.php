...
function executeRequest($historyID, $type='', $action='', $URL='', $requestData='', $timeout='', $isPluginResponse=true, $callOpt=array()){	
	...
	if($curlInfo['info']['http_code'] != 200 || !empty($curlInfo['errorNo'])){
		...
		if(!isPluginResponse($rawResponseData)){//sometimes 500 error with proper MultiAdmin Client Response, so if it not proper response continue set error and exit
	...
	if($isPluginResponse){//$isPluginResponse is set to true then expecting result should be pluginResponse
		...
		if(!isPluginResponse($rawResponseData)){ //Checking the timeout	
			...
			$updateHistoryAdditionalData['errorMsg'] = 'MultiAdmin Client plugin connection error.';
			updateHistory($updateHistoryData, $historyID, $updateHistoryAdditionalData);
			return checkTriggerStatus();	
		}else{
			if($type=='stats' && $action == 'getStats')
...
function updatesNotificationMailSend($force=false){
	...
	if($force == false){
		...
		//for proper notification send//updateOption('updatesNotificationMailLastSent', time());//AUTO MODE($force == false) This is to avoid re-tries for Mail sending which creates lot of getStats
	}
...
//			$requestData['args']['siteIDs'] = array(array_shift($siteIDs));
...
function copyToRequiredDir($source, $remoteSource, $destination){
	...
	if(empty($remoteSourceFiles)){
		appUpdateMsg('Unable to retrieve the directory list('.$remoteSource.').'.(($GLOBALS['FileSystemObj']->getMethod() == 'FTPExt') ? ' Please check APP_FTP_BASE folder path in config.php. It should point to the MultiAdmin root folder.' : ''), true);
		return false;
	}
...
function appUpdateMsg($msg, $isError=0){
	...
	if($isError === true){
		?>
        <br />Try again by refreshing the panel or contact <a href="mailto:help@arkinfotec.com">help@arkinfotec.com</a>
         <script>
...
function addNotification($type, $title, $message, $state='T', $callbackOnClose='', $callbackReference='', $scheduleTime=''){
	...
	if(!empty($_SESSION['userID']) && empty($_SESSION['offline']) && $scheduleTime == ''){//if user logged session
...
function getNotifications($reloaded=false){
	...
	//updateOption('offlineNotifications', NULL);//its updated in the above code
		...
		if(!empty($updateAddonsAvailable)){
			foreach($updateAddonsAvailable as $slug => $updateAddon){
				if(!empty($addons[$slug])){
					...
					//$addons[$slug]['isValidityExpired'] = $updateAddon['isValidityExpired'];//calculate isValidityExpired instead of getting from MultiAdmin Service(why? it can show less accurate data, because 24hrs once app check for update and get these data
...
function loadActiveAddons(){
	...
	$uninstallAddons = $uninstall = $activeLoadedAddonsSlugs = array();
...
function downloadAndInstallAddons($addons){
	...
	foreach($addons as $key => $addon){
		...
		$downloadCheckLink = getOption('serviceURL').'download.php?appInstallHash='.APP_INSTALL_HASH.'&installedHash='.$installedHash.'&appVersion='.APP_VERSION.'&type=addon&downloadType=install&download='.$addon['slug'].'&downloadVersion='.$addon['version'].'&downloadToken='.$_SESSION['downloadToken'];		
		if($currentGeneralSettings['participateBeta'] == 1){
...
function updateAppRegistered($user){//$user => username or email registered with MultiAdmin Site
...
function initMenus(){
	...
	$menus['manage']['subMenus'][] 		= array('page' => 'updates', 'displayName' => '<span class="float-left">Updates</span><span class="update_count float-left droid700 badge badge-danger" id="totalUpdateCount">0</span><div class="clear-both"></div>');
...
function onBrowserLoad(){
	unset($_SESSION['slowDownAjaxCallFrom']);
	installFolderAlert();
	cronNotRunAlert();	
}
