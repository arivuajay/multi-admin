...
class panelRequestManager{
	...
	public static function getClientUpdateAvailableSiteIDs(){
		...
		if(!empty($clientPluginBetaUpdate)){
			...
			if(!empty($clientPluginBetaUpdate['version']) && !empty($clientPluginBetaUpdate['downloadURL'])){
				if(empty($_SESSION['clientUpdates']['clientUpdateVersion']) || (!empty($_SESSION['clientUpdates']['clientUpdateVersion']) && version_compare($_SESSION['clientUpdates']['clientUpdateVersion'], $clientPluginBetaUpdate['version']) == -1)){
					$_SESSION['clientUpdates']['clientUpdateVersion'] = $clientPluginBetaUpdate['version'];
					$_SESSION['clientUpdates']['clientUpdatePackage'] = $clientPluginBetaUpdate['downloadURL'];			
				}
			}
		}
		...
		return (!empty($_SESSION['clientUpdates']['siteIDs']) ? $_SESSION['clientUpdates'] : false);
	...
	public static function generalCheck(&$finalResponse){
		...
		if($_SESSION['addonAlertCount'] !== $alertCount){
			$finalResponse['addonAlertCount'] = $_SESSION['addonAlertCount'] = $alertCount;
		}		
		
	}
	
	public static function updateHideNotify($version){//MultiAdmin update
	...
	public static function IWPAuthUser($params){
		...
		$registerURL = str_replace(array('service.', 'dev_service/', 'http://'), array('', '', 'https://'), $serviceURL);// to bring http://arkinfotec.com/
	...
	public static function getSendNextAjaxCallAfter(){
		...
		if($isTaskActive){
			unset($_SESSION['slowDownAjaxCallFrom']);//also called in onBrowserLoad() and this class handler
			return 0;
		}
		elseif(!empty($_SESSION['slowDownAjaxCallFrom'])) {
			if($_SESSION['slowDownAjaxCallFrom']['sec60'] < $time){
				return 60;
			}
			elseif($_SESSION['slowDownAjaxCallFrom']['sec30'] < $time){
				return 30;
			}
			elseif($_SESSION['slowDownAjaxCallFrom']['sec10'] < $time){
				return 10;
			}
			
		}
		else{
			$_SESSION['slowDownAjaxCallFrom']['sec10'] = $time + 12;//two calls of 10 sec
			$_SESSION['slowDownAjaxCallFrom']['sec30'] = $time + 35;//two calls of 30 sec
			$_SESSION['slowDownAjaxCallFrom']['sec60'] = $time + 105;//from there 60 sec each call
			return 0;
		}
