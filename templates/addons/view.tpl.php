...
if(!empty($appRegisteredUser)){ ?>
	<div style="padding: 10px;">Your arkinfotec.com account: <span style="font-weight: 700;"><?php echo $appRegisteredUser; ?></span></div>
<?php }
else{ ?>
	<div style="padding: 10px;">Your arkinfotec.com account: <span style="font-weight: 700;">You have not connected your account. </span> <a <?php if(!$d['isAppRegistered']){ ?>register="no" actionvar="register"<?php } ?> onclick="$('#checkNowAddons').click();">Connect Now</a></div>
<?php 
}
?>
...
<?php  }
	  ...
	  <div class="addons_empty_cont">You have installed all purchased addons / You have not purchased any new addons. You can purchase addons from the <a href="http://arkinfotec.com/addons/?utm_source=application&utm_medium=userapp&utm_campaign=purchaseAddon" target="_blank">SuperAdmin addon store</a>.</div>
  <?php } ?>
